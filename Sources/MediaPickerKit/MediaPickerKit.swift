//
//  MediaPickerKit.swift
//  Divar
//
//  Created by Faraz Karimi on 4/12/25.
//  Copyright © 2025 Divar. All rights reserved.
//
...
open class MediaPicker: UIViewController {
    ...
    private lazy var storeManager: RequestStoreManager = {
        let sm = RequestStoreManager(pickerConfig: config)
        return sm
    }()
    ...
    private func setupNavigationBar() {
        ...
        cancelButton.tintColor = .pickerTint
...
extension MediaPicker: UICollectionViewDataSource {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: AssetCollectionViewCell.id, for: indexPath) as? AssetCollectionViewCell,
...
extension MediaPicker: @preconcurrency MPAssetCollectionViewCellDelegate {
    ...
    public func didTapOnAsset(_ index: Int, isSelected: Bool) {
        ...
        if isSelected && (selectedAssets.count == config.limit) {
            self.delegate?.mediaPicker(self, didReachSelectionLimit: config.limit)
            return
        }
